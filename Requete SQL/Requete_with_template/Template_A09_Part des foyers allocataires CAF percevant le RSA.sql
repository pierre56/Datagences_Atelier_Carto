...
WITH table_data as (
  ...
  -- ajout indicateurs calculés
    sum(table_faits.nb_all_rsa)/sum(table_faits.nb_all_tot) AS pfoyrsa, -- part des foyers allocataires RSA
...
--		geographies.communes.the_geom
    FROM
        ...
        table_commune
    WHERE
        geographies.communes.id_commune = table_commune.id_commune_actuelle
    AND
        dimensions.communes.id_commune  = table_commune.id_commune_actuelle
